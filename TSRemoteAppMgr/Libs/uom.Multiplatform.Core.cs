...
return (SourceValue & BitMask) == BitMask;
	 ...
	 internal static int e_SetBitByMaskWithOffset(this int SourceValue, int iZeroBasedStartBitIndex, int iBitCountAtValue, int iValueToSet)
	 {
		...
		internal static class Extensions_Dictionary
		{
			...
			public static bool e_IsDictionaryEqualTo<TKey, TValue>(this Dictionary<TKey, TValue> dic1, Dictionary<TKey, TValue> dic2)
				=> dic1.OrderBy(x => x.Key).SequenceEqual(dic2.OrderBy(x => x.Key));
	...
	# End Region
		...
		internal static partial class Extensions_IO
		{
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			internal static string e_ReadToEnd(this FileInfo f, System.Text.Encoding? @encoding = null, bool detectEncodingFromByteOrderMarks = false)
			{
...
#pragma warning restore SYSLIB0014 // Type or member is obsolete
	...
	*/
		...
		internal static partial class Extensions_Security_Random
		{
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public static byte[] e_GetRandomBytesFast(this int count)
			{
			...
			/// <summary>Uses slow RNGCryptoServiceProvider</summary>
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public static byte[] e_GetRandomBytesRNG(this int count)
			{
				using RNGCryptoServiceProvider generatorProvider = new();
				var bytes = new byte[count];
				generatorProvider.GetBytes(bytes);
				return bytes;
			}
